...
if __name__ == "__main__":
    ...
    if args.folder == "":
        ...
        for action, status in action_and_status.items():

            # 影片的資料夾
            folder_original_data = "./violence_data/" + folder_name + "/" + action

            # 匯出的資料夾
            export_data = "./export_data/" + folder_name + "/"
            if not os.path.isdir(export_data):
                os.makedirs(export_data)

            value_csv = export_data + '/new_value_original_value.csv'
            if not os.path.isfile(value_csv):
                ...
                for frame in frame_provider:
                    ...
                    if poses_2d.shape[0] == 2:  # 人數為兩個人的時候才進判斷
                        ...
                        if len(history_features) == temp_frame_lenght_limit:
                            ...
                            features = violence.calculate_features_original_value(
                                history_features, closest_parts_distance)

                            for feature in features.values():  # 有8組
                                if feature != -1 and feature["status"] == status:
