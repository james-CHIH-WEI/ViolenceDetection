...
class Plotter3d:
    ...
    def mouse_callback(event, x, y, flags, params):
        ...
        if event == cv2.EVENT_MOUSEMOVE and should_rotate:
            ...
            phi -= (y - previous_position[1]) / \
                scale_dy * 6.2831 * 2  # 360 deg
...
def draw_poses(img, poses_2d):
    for pose_id in range(len(poses_2d)):  # 幾個人
        ...
        for kpt_id in range(pose.shape[1]):  # 0~18
            if pose[2, kpt_id] != -1:
                cv2.circle(img, tuple(pose[0:2, kpt_id].astype(
                    int)), 3, (0, 0, 255), -1, cv2.LINE_AA)


def draw_poses_two_color(img, poses_2d):
    for pose_id in range(len(poses_2d)):  # 幾個人
        pose = np.array(poses_2d[pose_id][0:-1]).reshape((-1, 3)).transpose()
        was_found = pose[2, :] > 0
        for edge in body_edges:
...
def draw_poses(img, poses_2d):
    for pose_id in range(len(poses_2d)):
        ...
        for edge in body_edges:
            if was_found[edge[0]] and was_found[edge[1]]:
                # print(tuple(pose[0:2, edge[0]].astype(int)))
                # print(tuple(pose[0:2, edge[1]].astype(int)))
                cv2.line(img, tuple(pose[0:2, edge[0]].astype(int)), tuple(pose[0:2, edge[1]].astype(int)),
                         (255, 0, 0), 4, cv2.LINE_AA)
        for kpt_id in range(pose.shape[1]):
            if pose[2, kpt_id] != -1:
                cv2.circle(img, tuple(pose[0:2, kpt_id].astype(int)), 3, (0, 0, 255), -1, cv2.LINE_AA)
'''
